				...
				choices: (answers) => {
					...
					const choice = table.getColumnsArray().map((column) => {
						...
						if (column.linkedName != undefined) {
							...
							let displayData = data[column.linkedName];

							if (displayData === undefined || displayData == null) displayData = "(none)"

							// return a string with linked name and corrosponding values
							return { name: `${column.linkedName}: ${displayData}`, value: column };
						} else {

							let displayData = answers.item[column.name];
							if (displayData === undefined || displayData == null) displayData = "(none)"

							// return a string with column names and corrosponding values
							return { name: `${column.name}: ${displayData}`, value: column };
						}
					}).filter((choice) => {
...
const deleteItem = async (tableName) => {
	try {
		...
		if (err.errno === 1451) {
			console.log(`could not delete ${tableName.slice(0, -1)} with data linked to it`);
		}
		else { throw err }
	}
}
